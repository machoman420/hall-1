...
<div id="confirmation" class="modal hide fade in" style="display: none; ">
	...
	<div class="modal-footer">
		<a  class="btn btn-success" id="add-student" data-dismiss="modal"> Confirm &amp; Update  </a>
...
<div class="span9 offset1"  id="content-area">
	<p id="hic"></p>
	<div class="span7">
		<form id="create-form" class="form-horizontal" action="<?php echo site_url('student'); ?>/check_update" method="post" accept-charset="utf-8" enctype="multipart/form-data">
			...
			<div class="control-group" id="div-std">
				...
				<div class="span4">
					<input name="student-no" id="student-no" type="text" class="input-xlarge" value="<?php echo $sid;?>"/>
				</div>
			</div>
			...
			if($("#stype").val()=='resident'){
				...
				<div class="span3">\
					<select name="room" id="room" class="input-xlarge">\
						...
						?>
						<option value="<?php echo $item->ID; ?>" <?php if($room==$item->ID)echo 'selected="selected"';?>> <?php echo $item->ID; ?></option>\
						<?php 	} ?>
					</select>\
				</div>\
			</div>';
				...
				<div class="span4">\
					<div id="dp" class="input-append date">\
						<input class="dp intput-xlarge" name="gdate" <?php if(isset($gdate)) echo 'value="'.$gdate.'"';?>/>\
					</div>\
				</div>\
			</div>').fadeIn('slow');
